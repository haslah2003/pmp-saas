...
type Ctx = { pptx: pptxgen; b: DeckBranding; iceOnNavy: string; illustrations: Record<number, string>; rtl: boolean };
...
function footer(ctx: Ctx, slide: pptxgen.Slide, siteFooter: string, dark: boolean) {
  slide.addText(siteFooter, {
    ...
    color: dark ? ctx.iceOnNavy : '9A9AAB', align: ctx.rtl ? 'right' : 'left', margin: 0,
  });
...
function speakerNotes(s: DeckSlide, spec: DeckSpec) {
  const cited = spec.citations.filter((citation) => s.citationRefs.includes(citation.ref));
  const sections: string[] = [];
  if (s.notes) sections.push(s.notes);
  if (cited.length) {
    sections.push(
      '[Sources]\n' + cited
        .map((citation) => `[${citation.ref}] ${citation.source_title} — ${citation.chunk_title} (${citation.framework})`)
        .join('\n')
    );
  }
...
function localized(ctx: Ctx, en: string, ar: string): string {
  return ctx.rtl ? ar : en;
}
...
function renderTitle(ctx: Ctx, slide: pptxgen.Slide, s: DeckSlide, spec: DeckSpec) {
  ...
  slide.addShape(ctx.pptx.ShapeType.roundRect, { x: 10, y: 0, w: 3.33, h: 3.4, rectRadius: 0.5, fill: { color: b.teal, transparency: 62 }, line: { type: 'none' } });
  slide.addShape(ctx.pptx.ShapeType.roundRect, { x: 11, y: 3.5, w: 2.33, h: 3.4, rectRadius: 0.45, fill: { color: b.violet, transparency: 66 }, line: { type: 'none' } });
  const textW = illo ? 6.9 : 9.7;
  chip(ctx, slide, 0.9, 1.5, s.kicker || spec.meta.pathwayLabel, false);
  slide.addText(s.headline || spec.title, { x: 0.85, y: 2.1, w: textW, h: 2.3, fontFace: b.fontHeading, fontSize: illo ? 38 : 42, bold: true, color: WHITE, lineSpacingMultiple: 1.0, align: ctx.rtl ? 'right' : 'left' });
  slide.addText(s.subhead || spec.subtitle, { x: 0.9, y: 4.55, w: textW, h: 0.9, fontFace: b.fontBody, fontSize: 17, color: ctx.iceOnNavy, lineSpacingMultiple: 1.1, align: ctx.rtl ? 'right' : 'left' });
...
function renderDefinition(ctx: Ctx, slide: pptxgen.Slide, s: DeckSlide, siteFooter: string) {
  ...
  chip(ctx, slide, 0.9, 0.6, s.kicker || localized(ctx, 'Definition', 'تعريف'), true);
  ...
  if (illo) {
    ...
    slide.addText(s.body || '', { x: 0.9, y: bodyY, w: 6.6, h: 5.0 - bodyY, fontFace: b.fontBody, fontSize: 17, color: b.bodyInk, lineSpacingMultiple: 1.2, valign: 'top', align: ctx.rtl ? 'right' : 'left' });
    illoCard(ctx, slide, illo, 7.95, 2.0, 4.5, 3.65, false);
  } else {
    slide.addText(s.body || '', { x: 0.9, y: 2.15, w: 7.2, h: 3.2, fontFace: b.fontBody, fontSize: 18, color: b.bodyInk, lineSpacingMultiple: 1.2, align: ctx.rtl ? 'right' : 'left' });
    if (s.stat) {
...
function renderOutcomes(ctx: Ctx, slide: pptxgen.Slide, s: DeckSlide, siteFooter: string) {
  const { b } = ctx;
  slide.background = { color: WHITE };
  chip(ctx, slide, 0.9, 0.6, s.kicker || localized(ctx, 'Intended outcomes', 'النتائج المستهدفة'), true);
  title(ctx, slide, s.headline, b.navy);
  ...
  items.forEach((it, i) => {
    ...
    if (it.title) slide.addText(it.title, { x: x + 1.2, y: y + 0.32, w: cw - 1.5, h: 0.7, fontFace: b.fontHeading, fontSize: 17, bold: true, color: b.navy, valign: 'middle', margin: 0, align: ctx.rtl ? 'right' : 'left' });
    slide.addText(it.desc, { x: x + 0.32, y: y + (it.title ? 1.2 : 0.35), w: cw - 0.6, h: it.title ? 0.8 : 1.5, fontFace: b.fontBody, fontSize: 14, color: b.bodyInk, margin: 0, lineSpacingMultiple: 1.05, valign: it.title ? 'top' : 'middle', align: ctx.rtl ? 'right' : 'left' });
  });
  footer(ctx, slide, siteFooter, false);
}
...
function renderProcess(ctx: Ctx, slide: pptxgen.Slide, s: DeckSlide, siteFooter: string) {
  const { b } = ctx;
  slide.background = { color: WHITE };
  chip(ctx, slide, 0.9, 0.6, s.kicker || localized(ctx, 'Process', 'العملية'), true);
  title(ctx, slide, s.headline, b.navy);
  const steps = (s.steps || []).slice(0, 5);
  const n = Math.max(steps.length, 1), x0 = 0.9, y = 2.9, gap = 0.35;
  const bw = (W - x0 * 2 - gap * (n - 1)) / n, bh = 1.7;
  steps.forEach((st, i) => {
    const visualIndex = ctx.rtl ? n - 1 - i : i;
    const x = x0 + visualIndex * (bw + gap);
    const dark = i % 2 === 0;
    ...
    if (i < n - 1) slide.addText(ctx.rtl ? '‹' : '›', { x: ctx.rtl ? x - gap - 0.02 : x + bw - 0.02, y: y + 0.45, w: gap + 0.04, h: 0.9, fontFace: b.fontHeading, fontSize: 26, bold: true, color: b.teal, align: 'center', valign: 'middle', margin: 0 });
  });
  if (s.caption) slide.addText(s.caption, { x: 0.9, y: 5.0, w: 11.5, h: 0.6, fontFace: b.fontBody, fontSize: 15, italic: true, color: b.violet, align: ctx.rtl ? 'right' : 'left' });
  footer(ctx, slide, siteFooter, false);
}
...
function renderLevels(ctx: Ctx, slide: pptxgen.Slide, s: DeckSlide, siteFooter: string) {
  const { b } = ctx;
  slide.background = { color: b.navy };
  chip(ctx, slide, 0.9, 0.55, s.kicker || localized(ctx, 'Levels', 'المستويات'), false);
  ...
  levels.forEach((lv, i) => {
    ...
    slide.addText(lv.name, { x: 1.1, y, w: 2.4, h: rh, fontFace: b.fontHeading, fontSize: 15, bold: true, color: WHITE, valign: 'middle', margin: 0, align: ctx.rtl ? 'right' : 'left' });
    slide.addText(lv.desc, { x: 3.4, y, w: Math.max(wStep - 2.6, 1.5), h: rh, fontFace: b.fontBody, fontSize: 12.5, color: ctx.iceOnNavy, valign: 'middle', margin: 0, align: ctx.rtl ? 'right' : 'left' });
  });
...
function renderTwoColumn(ctx: Ctx, slide: pptxgen.Slide, s: DeckSlide, siteFooter: string) {
  const { b } = ctx;
  slide.background = { color: WHITE };
  chip(ctx, slide, 0.9, 0.6, s.kicker || localized(ctx, 'Application', 'التطبيق'), true);
  title(ctx, slide, s.headline, b.navy);
  ...
  const col = (x: number, colTitle: string, items: string[], accent: string) => {
    ...
    slide.addText(colTitle, { x: x + 0.35, y: y + 0.3, w: colW - 0.7, h: 0.55, fontFace: b.fontHeading, fontSize: 19, bold: true, color: accent, margin: 0, align: ctx.rtl ? 'right' : 'left' });
    slide.addText((items || []).map((t, i, arr) => ({ text: t, options: { bullet: { code: '2022' }, breakLine: i < arr.length - 1 } })), { x: x + 0.4, y: y + 1.0, w: colW - 0.75, h: colH - 1.3, fontFace: b.fontBody, fontSize: 15, color: b.bodyInk, paraSpaceAfter: 10, lineSpacingMultiple: 1.02, margin: 0, align: ctx.rtl ? 'right' : 'left' });
  };
  col(ctx.rtl ? x2 : x1, s.left_title || localized(ctx, 'Do', 'افعل'), s.left || [], b.teal);
  col(ctx.rtl ? x1 : x2, s.right_title || localized(ctx, 'Avoid', 'تجنب'), s.right || [], b.purple);
  footer(ctx, slide, siteFooter, false);
}
...
function renderExamFocus(ctx: Ctx, slide: pptxgen.Slide, s: DeckSlide, siteFooter: string) {
  const { b } = ctx;
  slide.background = { color: WHITE };
  chip(ctx, slide, 0.9, 0.6, s.kicker || localized(ctx, 'Exam focus', 'تركيز الاختبار'), true);
  title(ctx, slide, s.headline, b.navy);
  ...
  items.forEach((it, i) => {
    ...
    const badgeX = ctx.rtl ? W - 1.74 : 1.1;
    badge(ctx, slide, badgeX, y + 0.14, 0.64, '✓', b.teal);
    slide.addText(it, { x: ctx.rtl ? 1.1 : 2.0, y, w: 10.2, h: rh, fontFace: b.fontBody, fontSize: 16, color: b.navy, valign: 'middle', margin: 0, align: ctx.rtl ? 'right' : 'left' });
  });
  footer(ctx, slide, siteFooter, false);
}
...
function renderClosing(ctx: Ctx, slide: pptxgen.Slide, s: DeckSlide, spec: DeckSpec, siteFooter: string) {
  ...
  const illo = ctx.illustrations[s.n];
  slide.background = { color: b.navy };
  slide.addShape(ctx.pptx.ShapeType.roundRect, { x: 0, y: 4.1, w: 3.6, h: 3.4, rectRadius: 0.5, fill: { color: b.violet, transparency: 64 }, line: { type: 'none' } });
  if (!illo) slide.addShape(ctx.pptx.ShapeType.roundRect, { x: 10.6, y: 0, w: 2.73, h: 2.6, rectRadius: 0.4, fill: { color: b.teal, transparency: 66 }, line: { type: 'none' } });
  const textW = illo ? 6.7 : 10.6;
  chip(ctx, slide, 0.9, 1.2, s.kicker || localized(ctx, 'Key takeaway', 'الخلاصة الرئيسية'), false);
  slide.addText(s.headline, { x: 0.85, y: 1.75, w: textW, h: 0.9, fontFace: b.fontHeading, fontSize: 34, bold: true, color: WHITE, align: ctx.rtl ? 'right' : 'left' });
  slide.addText(s.body || '', { x: 0.9, y: 2.85, w: textW, h: 2.1, fontFace: b.fontBody, fontSize: 20, color: ctx.iceOnNavy, lineSpacingMultiple: 1.2, align: ctx.rtl ? 'right' : 'left' });
  ...
  if (spec.citations.length) {
    ...
    slide.addText(src, { x: 0.9, y: 6.35, w: 11.5, h: 0.5, fontFace: b.fontBody, fontSize: 9, color: ctx.iceOnNavy, margin: 0, align: ctx.rtl ? 'right' : 'left' });
  }
  footer(ctx, slide, siteFooter, true);
}
...
export async function buildDeckPptx(
  ...
  pptx.rtlMode = spec.meta.locale === 'ar';
  pptx.author = `${branding.siteName} — Deck Builder`;
  pptx.company = branding.siteName;
  pptx.title = spec.title;

  const ctx: Ctx = { pptx, b: branding, iceOnNavy: lighten(branding.violet, 0.72), illustrations, rtl: spec.meta.locale === 'ar' };
  const siteFooter = spec.meta.locale === 'ar'
    ? `${branding.siteName} · موثّق بمصادر ${spec.meta.pathwayLabel}`
    : `${branding.siteName} · Grounded in ${spec.meta.pathwayLabel}`;
  ...
  for (const s of spec.slides) {
    ...
    const notes = speakerNotes(s, spec);
