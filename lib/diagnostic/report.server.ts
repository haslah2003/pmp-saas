...
export async function loadIndividualReport(sessionId: string, candidateId: string) {
  ...
  const { data: session } = await admin.from('diagnostic_sessions').select('id,form_id,status,locale,started_at,submitted_at').eq('id', sessionId).eq('candidate_id', candidateId).maybeSingle();
  ...
  return {
    ...
    report: {
      ...report,
      learner: {
        fullName: String(fullName || 'Guest learner'),
