    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4" dir={dir}>
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">{t(locale, 'auth.welcome_back')}</h1>
          <p className="text-gray-500 mt-1">{t(locale, 'auth.sign_in_continue')}</p>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          <button
            onClick={handleGoogleLogin}
            className="w-full flex items-center justify-center gap-3 border-2 border-gray-200 rounded-xl py-3 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition mb-6"
          >
            ...
            {t(locale, 'auth.continue_google')}
          </button>

          <div className="flex items-center gap-3 mb-6">
            <div className="flex-1 h-px bg-gray-200" />
            <span className="text-xs text-gray-400 uppercase">{t(locale, 'auth.or')}</span>
            <div className="flex-1 h-px bg-gray-200" />
          </div>

          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t(locale, 'auth.email')}</label>
              <input
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t(locale, 'auth.password')}</label>
              <input
            ...
            >
              {loading ? t(locale, 'auth.signing_in') : t(locale, 'auth.sign_in')}
            </button>
          </form>
        </div>
