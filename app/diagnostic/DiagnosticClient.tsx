    ...
    <main dir={ar ? 'rtl' : 'ltr'} className="min-h-screen bg-slate-50 px-4 py-5 sm:py-9">
      <section className="mx-auto max-w-3xl">
        ...
        <article className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
          ...
          <h1 className="mt-6 text-lg font-semibold leading-8 text-slate-950" lang={ar ? 'ar' : 'en'} dir={ar ? 'rtl' : 'ltr'}>{current?.stem}</h1>
          ...
          <div className="mt-6 space-y-3" role={current?.itemType === 'multiple_response' ? 'group' : 'radiogroup'} aria-label={ar ? 'خيارات الإجابة' : 'Answer options'}>{current?.positionOptions.map((option) => <label key={option.id} className={`flex cursor-pointer gap-3 rounded-xl border-2 p-4 ${ar ? 'text-right' : 'text-left'} ${answers[current.id]?.includes(option.id) ? 'border-teal-600 bg-teal-50' : 'border-slate-200 bg-white'}`} dir={ar ? 'rtl' : 'ltr'}><input type={current.itemType === 'multiple_response' ? 'checkbox' : 'radio'} name="answer" value={option.id} checked={answers[current.id]?.includes(option.id) || false} onChange={() => selectAnswer(option.id)} className="mt-1" /><span><b className={ar ? 'ml-2' : 'mr-2'}>{option.id}.</b>{option.text}</span></label>)}</div>
        </article>
        {error && <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}
